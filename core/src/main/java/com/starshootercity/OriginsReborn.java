...
public class OriginsReborn extends OriginsAddon {
    ...
    private static void initializeNMSInvoker(OriginsReborn instance) {
        nmsInvoker = switch (Bukkit.getMinecraftVersion()) {
            ...
            case "1.20.5" -> new NMSInvokerV1_20_5();
            case "1.20.6" -> new NMSInvokerV1_20_6();
    ...
    public void updateConfig() {
        ...
        if (version.equals("1.0.0")) saveResource("config.yml", true);
            ...
            if (version.equals("2.1.14")) {
                getConfig().set("config-version", "2.1.11");
                getConfig().setComments("origin-selection.show-initial-gui", null);
            ...
            if (version.equals("2.1.16")) {
                ...
                getConfig().setComments("origin-selection.invulnerable-mode", List.of(
            ...
            if (version.equals("2.1.19")) {
                ...
                getConfig().setComments("messages.no-swap-command-permissions", List.of("Player used /origin swap and does not have permission"));
                getConfig().setComments("messages", List.of("Configure plugin messages", "Create an issue on the Origins-Reborn Github if you'd like to add a configuration option for a message"));
                saveConfig();
            }
            if (version.equals("2.1.20")) {
                getConfig().set("config-version", "2.2.3");
                getConfig().set("misc-settings.disable-flight-stuff", false);
                getConfig().setComments("misc-settings", List.of("Miscellaneous settings"));
                getConfig().setComments("misc-settings.disable-flight-stuff", List.of("Disable all flight-related features. This does not hide the abilities themselves, they must be removed from the .yml files in the ~/plugins/Origins-Reborn/origins/ folder"));
                saveConfig();
            }
            if (version.equals("2.2.3")) {
                getConfig().set("config-version", "2.2.5");
                getConfig().set("geyser.join-form-delay", 20);
                getConfig().setComments("geyser", List.of("Settings for using GeyserMC"));
            ...
            if (version.equals("2.1.11") || version.equals("2.1.10")) {
                ...
                getConfig().setComments("display.force-unicode-font", List.of(
